  ...
  const getActivityStatus = () => {
    ...
    return formatDate(collection.stats.lastActivity)
      ...
      return (
        <div
          onClick={onClick}
          className="bg-white dark:bg-[#151517] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-200 dark:border-gray-800 hover:shadow-[0_8px_20px_-4px_rgba(0,0,0,0.08)] hover:border-[#C9A24D]/30 dark:hover:border-[#C9A24D]/50 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer group flex flex-col h-full overflow-hidden"
        >
          {/* Header Section */}
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-4 min-w-0">
                ...
                <div className="relative flex-shrink-0">
                  <div className="w-12 h-12 bg-[#111827] dark:bg-white rounded-2xl flex items-center justify-center shadow-sm transition-transform duration-300">
                    <span className="text-white dark:text-[#111827] text-lg font-black">
                      {collection.customer.firstName.charAt(0)}{collection.customer.lastName.charAt(0)}
                    </span>
                  </div>
                  <div className="absolute -top-1 -right-1 w-4 h-4 bg-[#C9A24D] rounded-full border-2 border-white dark:border-[#151517] shadow-sm"></div>
                </div>
                
                <div className="min-w-0">
                  <h3 className="font-black text-[#0B0B0B] dark:text-white text-lg tracking-tight group-hover:text-[#C9A24D] transition-colors truncate">
                    {collection.customer.firstName} {collection.customer.lastName}
                  </h3>
                  <p className="text-sm text-[#6B7280] dark:text-gray-400 font-medium truncate">{collection.customer.email}</p>
                </div>
              </div>
          ...
          <div className="flex items-center space-x-1">
            {onEditPreferences && (
              <button
                ...
                className="p-2 text-gray-400 dark:text-gray-500 hover:text-[#C9A24D] dark:hover:text-[#C9A24D] hover:bg-[#FAFAF7] dark:hover:bg-gray-800 rounded-xl transition-all"
            ...
            {onShare && (
              <button
                ...
                className="p-2 text-gray-400 dark:text-gray-500 hover:text-[#111827] dark:hover:text-white hover:bg-[#FAFAF7] dark:hover:bg-gray-800 rounded-xl transition-all"
            ...
            {onDelete && (
              <button
                ...
                className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all"
          ...
          {/* Metrics Section */}
          <div className="px-6 py-4 bg-[#FAFAF7] dark:bg-[#0B0B0B] border-y border-gray-100 dark:border-gray-800 group-hover:bg-white dark:group-hover:bg-[#151517] transition-colors duration-300">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col border-l border-gray-200 dark:border-gray-800 pl-4">
                <span className="text-[10px] font-black text-[#6B7280] dark:text-gray-400 uppercase tracking-widest mb-1">Budget Range</span>
                <div className="text-sm font-black text-[#C9A24D] leading-tight bg-[#C9A24D]/5 dark:bg-[#C9A24D]/10 px-2 py-1 rounded border border-[#C9A24D]/10 dark:border-[#C9A24D]/20 inline-block w-fit transition-colors group-hover:bg-[#C9A24D]/10">
      ...
      {(collection.stats.totalProperties >= 41 || collection.stats.totalProperties === 0) && (
        <div className={`px-6 py-2 border-b ${collection.stats.totalProperties === 0 ? 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-900' : 'bg-amber-50 dark:bg-amber-900/20 border-amber-100 dark:border-amber-900'}`}>
        ...
        <div className="space-y-4">
          ...
          {collection.preferences && (
            <div className="flex flex-wrap gap-1.5">
              {(collection.preferences as any).is_single_family && <span className="px-2 py-1 rounded-lg text-[10px] font-bold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-[#6B7280] dark:text-gray-400 shadow-sm">Single Family</span>}
              {(collection.preferences as any).is_town_house && <span className="px-2 py-1 rounded-lg text-[10px] font-bold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-[#6B7280] dark:text-gray-400 shadow-sm">Townhouse</span>}
              {(collection.preferences as any).is_condo && <span className="px-2 py-1 rounded-lg text-[10px] font-bold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-[#6B7280] dark:text-gray-400 shadow-sm">Condo</span>}
      ...
      {/* Footer */}
      <div className="px-6 py-3 bg-white dark:bg-[#151517] border-t border-gray-50 dark:border-gray-800 mt-auto">
        <div className="flex items-center justify-between text-[10px] font-bold text-[#6B7280] dark:text-gray-400 uppercase tracking-widest">
