    ...
    if (targetId) {
       ...
       if (mode === 'recommendations' && targetOH) {
          ...
          if (id && (targetOH as any).similarPropertiesSnapshot) {
            ...
            currentSimilarProps = await fetchSimilarProperties(targetOH, true)
          } else {
            // New creation wizard: use state if available, or fetch if not
            if (similarProperties.length === 0) {
              currentSimilarProps = await fetchSimilarProperties(targetOH, false)
            } else {
              currentSimilarProps = similarProperties;
            }
            
            // For new wizard, we MUST filter by user selection
            currentSimilarProps = currentSimilarProps.filter(p => {
